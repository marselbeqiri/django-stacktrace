...
class CrashEventAdmin(admin.ModelAdmin):
    ...
    def error_summary(self, obj):
        ...
        return format_html(
            '<div class="crash-error-type"><a href="{}">{}</a></div>'
            '<div class="crash-culprit">{}</div>'
            '<div class="crash-message">{}</div>'
            '<span class="crash-event-id">{}</span>',
            f"{obj.pk}/change/",
            error_type,
            culprit,
            message,
    ...
    def level_badge(self, obj):
        ...
        css_class = f"crash-level crash-level-{level}"
        return format_html(
            '<span class="{}">{}</span>',
            css_class,
            level.upper(),
    ...
    def events_count(self, obj):
        ...
        css_class = "crash-count crash-count-high" if count >= 1000 else "crash-count"
    ...
    def time_info(self, obj):
        ...
        return format_html(
            '<div class="crash-timestamp">{} ago</div>'
            '<div class="crash-timestamp">{} old</div>',
            last_seen_ago,
            age,
        )
    ...
    def logger_display(self, obj):
        ...
        if server:
            return format_html(
                '<div class="crash-logger">{}</div>'
                '<div class="crash-server">{}</div>',
                logger,
                server,
            )
        return format_html('<span class="crash-logger">{}</span>', logger)
    ...
    def request_info(self, obj):
        ...
        if method:
            return format_html(
                "<div><strong>{}</strong></div>" '<div class="crash-path">{}</div>',
                method,
                path_display,
            )
        return format_html('<span class="crash-path">{}</span>', path_display)
