...
from django_crashlog.event_store.store import store_crash_event


class EventStoreTests(SimpleTestCase):
    def test_store_crash_event_disabled_returns_none(self):
        with (
            patch("django_crashlog.event_store.store.CRASHLOG_ENABLED", False),
            patch(
                "django_crashlog.event_store.store.build_crash_event"
    ...
    def test_store_crash_event_skips_when_sampling_or_rate_limit_blocks(self):
        with (
            patch(
                "django_crashlog.event_store.store.should_sample_event",
                return_value=False,
            ),
            patch(
                "django_crashlog.event_store.store.is_within_rate_limit",
                return_value=True,
            ),
        ):
            result = store_crash_event()
    ...
    def test_store_crash_event_creates_new_record_when_no_existing_hash(self):
        ...
        with (
            ...
            patch(
                "django_crashlog.event_store.store.build_crash_event_model_data",
                return_value=model_data,
            ),
            patch("django_crashlog.event_store.store.CrashEvent.objects", objects),
        ):
            result = store_crash_event()
